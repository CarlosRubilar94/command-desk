...
def _command_deck_base_url() -> str:
    """Resolve Command Deck (control-center) base URL for DevSSD probes."""
    raw = (os.environ.get("COMMAND_DECK_URL") or _COMMAND_DECK_DEFAULT_BASE).strip()
    return raw.rstrip("/")
...
def _resolve_gateway_liveness() -> tuple[bool, str | None]:
    """Compact gateway snapshot shared by DevSSD status."""
    gateway_pid = get_running_pid()
    gateway_running = gateway_pid is not None
    remote_health_body: dict | None = None
    ...
    return gateway_running, gateway_state


def _devssd_cli_invocation() -> str:
    for name in ("command-desk", "hermes"):
        if shutil.which(name):
            return name
    return f"{sys.executable} -m hermes_cli.main"


def _devssd_smoke_commands(cli: str) -> dict[str, str]:
    def _cmd(*parts: str) -> str:
        return f"{cli} {' '.join(parts)}"

    return {
        "version": _cmd("--version"),
        "doctor": _cmd("doctor"),
        "gateway-status": _cmd("gateway", "status"),
        "bitwarden-status": _cmd("secrets", "bitwarden", "status"),
        "one-shot": _cmd("-z", "Reply OK only", "--cli"),
    }


def _devssd_skill_installed(skills_dir: Path) -> bool:
    candidates = (
        skills_dir / "devssd-ops" / "SKILL.md",
        PROJECT_ROOT / "skills" / "devssd-ops" / "SKILL.md",
        PROJECT_ROOT / "optional-skills" / "devssd-ops" / "SKILL.md",
    )
    return any(path.is_file() for path in candidates)


def _build_bitwarden_status_payload(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    server_url = str(bw_cfg.get("server_url") or "").strip() or "https://vault.bitwarden.com"
    token_present = bool(os.environ.get(token_env, "").strip())
    project_configured = bool(project_id)
    warnings: list[str] = []

    if enabled and not token_present:
        warnings.append(f"Enabled but {token_env} is not set.")
    if enabled and not project_configured:
        warnings.append("Enabled but project_id is not configured.")
    ...
    payload: dict[str, Any] = {
        ...
        "bws_found": bool(binary),
        "bws_path": str(binary) if binary else None,
        "secret_count": None,
        "error": None,
        "warnings": warnings,
    }
...
@app.get("/api/devssd/status")
async def get_devssd_status():
    """Protected DevSSD operational snapshot for Command Desk pages.

    Reuses gateway liveness helpers from ``/api/status`` and probes Command
    Deck (:8765) for cross-service health. No secret values are returned.
    """
    cfg = load_config()
    ...
    cli = _devssd_cli_invocation()

    gateway_running, gateway_state = _resolve_gateway_liveness()
    loop = asyncio.get_running_loop()
    command_deck = await loop.run_in_executor(None, _probe_command_deck_status)
    ...
    return {
        ...
        "gateway_state": gateway_state,
        "dashboard_up": True,
        "command_deck_up": command_deck["available"],
        "agent": {
            "model": model,
            "provider": provider,
            "home": str(home),
            ...
            "devssd_skill_installed": _devssd_skill_installed(skills_dir),
            "commands": _devssd_smoke_commands(cli),
        },
        "bitwarden": _build_bitwarden_status_payload(cfg),
        "gateway": {
            "running": gateway_running,
            "health_url": _GATEWAY_HEALTH_URL,
        },
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "command-desk-ops",       "label": "Command Deck Ops",         "description": "Dark ops console — DevSSD cyan accent on #070B12"},
    {"name": "command-desk-ops-large", "label": "Command Deck Ops (Large)", "description": "Ops console with bigger fonts and roomier spacing"},
    {"name": "default",       "label": "Command Deck Ops",         "description": "Dark ops console — DevSSD cyan accent on #070B12"},
    {"name": "default-large", "label": "Command Deck Ops (Large)", "description": "Ops console with bigger fonts and roomier spacing"},
