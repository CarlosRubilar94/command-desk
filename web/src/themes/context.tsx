...
const THEME_NAME_ALIASES: Record<string, string> = {
  ...
  "lens-5i": "nous-blue",
  // DevSSD Command Deck Ops is the canonical dashboard look.
  default: "command-desk-ops",
  "default-large": "command-desk-ops-large",
  midnight: "command-desk-ops",
  ember: "command-desk-ops",
  mono: "command-desk-ops",
  cyberpunk: "command-desk-ops",
  rose: "command-desk-ops",
  "nous-blue": "command-desk-ops",
};
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [themeName, setThemeName] = useState<string>(() => {
    if (typeof window === "undefined") return "command-desk-ops";
    const stored = window.localStorage.getItem(STORAGE_KEY) ?? "command-desk-ops";
  ...
  const setTheme = useCallback(
    (name: string) => {
      ...
      const next = knownNames.has(name) ? name : "command-desk-ops";
      setThemeName(next);
      if (typeof window !== "undefined") {
