...
import { BUILTIN_THEMES, THEME_DEFAULT_FONT_ID, PICKER_THEME_NAMES, useTheme } from "@/themes";
...
export function ThemeSwitcher({ collapsed = false, dropUp = false }: ThemeSwitcherProps) {
  ...
  const pickerThemes = availableThemes.filter((th) => PICKER_THEME_NAMES.has(th.name));
